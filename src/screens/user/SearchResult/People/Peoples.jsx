...
class People extends Component {
  ...
  renderData() {
    if (this.props?.typeAction === 'ME') {
      return this.props?.followers?.followeds?.map(({ user_follower }, index) => (
        <UserFlowCard typeAction={'remove'} {...user_follower} key={index} />
      ));
    } else if (this.props?.typeAction === 'There') {

      return this.props?.followers?.followers?.map((item) => {
  ...
  render() {
    {console.log("foll", this.props.followers);}
    return (
      <WrapperWithTitle>
...
const mapStateToProps = (state) => ({
  followers: state.userDetailes.userProfile,
